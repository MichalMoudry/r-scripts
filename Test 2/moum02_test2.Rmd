...
```{r}
scatter.smooth(pracovnici_data$plat, pracovnici_data$delka_praxe)
```
...
Pro otestování významnosti regresního parametru b1 je třeba spočítat t-hodnotu a podle toho, jestli spadá/nespadá do testového kritéria (Wα), tak buď zamítáme ve prospěch alternativy nebo ne.
  ...
  - Alternativa je, jestli je lineární závislost statisticky významná.
```{r}
summary_fit_pracovnici
```
`Odpověď`:
  - t-hodnota: 
...
```
`Odpověď`: V tomto případě hodnota koeficientu determinace je 1.817667e-05, tedy daný model je špatný, tedy špatně vysvětluje variabilitu v souboru. Taky tento model špatně predikuje reálné datové body.
...
`Odpověď`:
  ...
  - Kritický obor: Wα = {t <= -t1-α} = {-1.370828 <= -2.326348}
